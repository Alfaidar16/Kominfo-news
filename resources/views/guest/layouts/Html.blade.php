                    ...
                    <div class="mobile-menu-items">
                        ...
                        <ul class="sub-menu" >
                            ...
                            {{-- <li class="menu-item-has-children active">
                                ...
                                <ul>
                                    @php
                                        $subMenu1 =  DB::table('menu_items')->where('parent_id','=', 17)->orderBy('id', 'DESC')->get();
                                        $subMenu2 = DB::table('menu_items')->where('parent_id', '=', 108)->orderBy('id', 'DESC')->get();
                                    @endphp
                                     @foreach ($subMenu1 as $key )
                                     <li class="dropdown-item dropdown-submenu"><a href="{{ $key->url}}"target="_blank">{{$key->title}}</a></li>
                                     @if($key->title = 'cobamenu')
                                     <ul class="submenu dropdown-item" aria-labelledby="hom1">
                                        @foreach ($subMenu2 as $v )
                                        <li> 
                                            <a  href="" class="dropdown-item test">{{$v->title}}</a>
                                        </li>
                                     @endforeach
